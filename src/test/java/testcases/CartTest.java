...
public class CartTest extends BaseClass {
	...
	public void testGetAllCarts() {
		given().when().get(Routes.GET_ALL_CARTS).then().statusCode(200).body("size()", greaterThan(0)); // Validate that
																										// the response
																										// is not empty
	}
	...
	public void testGetCartById() {
		...
		given().pathParam("id", cartId).when().get(Routes.GET_CART_BY_ID).then().statusCode(200).log().body().body("id",
				equalTo(cartId)); // Validate that the response contains the correct cart ID
	}
	...
	public void testGetCartsByDateRange() {
		...
		Response response = given().pathParam("startdate", startDate).pathParam("enddate", endDate).when()
				.get(Routes.GET_CARTS_BY_DATE_RANGE).then().statusCode(200).body("size()", greaterThan(0)) // Validate
																											// that the
																											// response
																											// is not
																											// empty
	...
	public void testGetUserCart() {
		...
		given().pathParam("userId", userId).when().get(Routes.GET_USER_CART).then().statusCode(200).body("userId",
				everyItem(equalTo(userId)));
